...
int taskFlag[MAXN];// 记录每个机器所匹配到的位置
bool taskUsed[n];
set<int> doneSet;
        ...
        for(int ii=0; ii<m; ii++)
        {
            ...
            if(isdepcopy[flag_end[ii]][position]==0)
            {
                ...
                for(int kk=0; kk<n; kk++)
                {
                    ...
                    {
                        ltemp=taoend[flag_end[ii]][kk]+c[kk][position];
                        if(ltemp>Lstart)
                        {
                            Lstart=ltemp;
                        }
...
void insertMachine(Individual *i, int a, int b, int c) { // a = taskFlag[nowPoint], b = nowPoint, c = temp
    vector<int>:: iterator iter;
...
int find_ele(Individual *i, int point, int index) {
    int j = 0;
    vector<int>:: iterator iter;
    iter = i->machine[point].begin();
    while(j < index) {
        iter ++;
        j ++;
    }
...
void repair_segment(Individual *i) {
    /***************initialize******************/
    vector<int>:: iterator iter;
    memset(taskFlag, 0, sizeof(taskFlag));
    int acTask = 0;
    int nowPoint = 0;
    int temp;
    ...
    while(acTask < n) {
        ...
        for(nowPoint = 0 ; nowPoint < m ; nowPoint ++) { // 对所有的指针进行循环
            printf("nowPoint = %d\n", nowPoint);
            int nowTask = find_ele(i, nowPoint, taskFlag[nowPoint]);
            printf("nowTask = %d\n", nowTask);
//            printf("nowPoint = %d\n", nowPoint);
            temp = test(nowTask);
            printf("temp = %d\n", temp);
            if(temp == -1) { // 如果能符合依赖，指针后移并且可满足的机器数+1.
                taskFlag[nowPoint] ++;
                acTask ++;
        ...
        nowPoint = rand() % m;
        //insertMachine(i, taskFlag[nowPoint], nowPoint, temp);
    }
}
